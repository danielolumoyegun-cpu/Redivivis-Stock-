/* ============================================================
   Storage — localStorage data layer for audit runs & evidence
   ============================================================ */
(function (global) {
  'use strict';

  const DATA_KEY     = 'rvs_data';
  const SETTINGS_KEY = 'rvs_settings';
  ...
  function load() {
    try {
      return JSON.parse(localStorage.getItem(DATA_KEY) || '{"runs":[]}');
    } catch (_) {
      return { runs: [] };
    }
  }
  ...
  function getActiveRun() {
    return load().runs.find(r => r.status === 'active') || null;
  }
  ...
  function completeRun(id) {
    return updateRun(id, { status: 'complete', completedAt: new Date().toISOString() });
  }
